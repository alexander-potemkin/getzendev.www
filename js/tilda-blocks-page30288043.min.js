...
function t650_init(recId){t650_moveClickOnCard(recId)}
function t650_unifyHeights(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var wrapper=rec.querySelector('.t650');if(!wrapper)return;var container=rec.querySelector('.t650__container');var columnsInRow=Number(container.getAttribute('data-columns-in-row'));if(columnsInRow===0||columnsInRow===''){columnsInRow=1}
var allCols=Array.prototype.slice.call(rec.querySelectorAll('.t650__col'));var rowsArray=[];for(var i=0;i<allCols.length;i+=columnsInRow){rowsArray.push(allCols.slice(i,i+columnsInRow))}
...
function t650_moveClickOnCard(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var cards=rec.querySelectorAll('.t650__col');if(!cards)return;Array.prototype.forEach.call(cards,function(card){var downEventTime;var upEventTime;var link=card.querySelector('.t650__link');if(!link)return;card.style.cursor='pointer';card.addEventListener('mousedown',function(){downEventTime=Date.now()});card.addEventListener('mouseup',function(){upEventTime=Date.now();if((upEventTime-downEventTime)<200){link.click()}})})}
