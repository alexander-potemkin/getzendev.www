...
function t844_init(recId){t844_moveClickOnCard(recId)}
function t844_moveClickOnCard(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var cards=rec.querySelectorAll('.t844__col');if(!cards)return;Array.prototype.forEach.call(cards,function(card){var downEventTime;var upEventTime;var link=card.querySelector('.t844__link');if(!link)return;card.style.cursor='pointer';card.addEventListener('mousedown',function(){downEventTime=Date.now()});card.addEventListener('mouseup',function(){upEventTime=Date.now();if((upEventTime-downEventTime)<200){link.click()}})})}
function t923_init(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var container=rec.querySelector('.t923');if(!container)return;t923_unifyHeights(recId);window.addEventListener('resize',t_throttle(function(){t923_unifyHeights(recId)}));container.addEventListener('displayChanged',function(){t923_unifyHeights(recId)});window.addEventListener('load',function(){t923_unifyHeights(recId)})}
